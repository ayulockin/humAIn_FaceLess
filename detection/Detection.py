			...
			for detection in output:
				...
				if classID == 0:
					...
					if confidence > self.confidence:
						...
						humans.append(classID)
			
		if self.bbox is True:
			frame = self.drawBoundingBox(frame, boxes, confidences, humans)
			return frame
		else:
			return len(humans)

	def drawBoundingBox(self, frame, boxes, confidences, humans):
		...
		if len(idxs) > 0:
			for i in idxs.flatten():
				...
				color = [int(c) for c in self._COLORS[humans[i]]]
				cv2.rectangle(frame, (x, y), (x + w, y + h), color, 2)
				text = "{}: {:.4f}".format(self._LABELS[humans[i]],
					confidences[i])
